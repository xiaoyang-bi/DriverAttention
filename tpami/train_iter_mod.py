...
def run_infer_mixup(args, model, data_loader, gaze_average, device, topK=16):
    model.eval()
    gaze_average = gaze_average.to(device)
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    os.makedirs(str(Path(args.data_path)/'mix_data2'), exist_ok=True)
    os.makedirs(str(Path(args.data_path)/'mix_data2'/'0'), exist_ok=True)
    os.makedirs(str(Path(args.data_path)/'mix_data2'/'1'), exist_ok=True)
    os.makedirs(str(Path(args.data_path)/'mix_data2'/'camera_224_224'), exist_ok=True)
            ...
            for idx in top_kl_idx:
                for j in range(batch_size):
                    ...
                    for p_idx, mix_p in enumerate(mix_ps):
                        ...
                        cv2.imwrite( str(Path(args.data_path)/'mix_data2'/'{}'.format(p_idx)/"{}.jpg".format(count)), p_out) 

                    cv2.imwrite( str(Path(args.data_path)/'mix_data2'/'camera_224_224'/"{}.jpg".format(count)), img_out)
                    count += 1
...
def main(args):
    ...
    for epoch in tqdm(range(args.start_epoch, args.epochs)):
        ...
        if epoch == 0:
            gaze_average = convert('bd_mix_gaze_average.jpg')
            gaze_average = gaze_average.cuda()
