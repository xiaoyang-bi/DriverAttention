    ...
    with torch.no_grad():
        ...
        for images, p, out_path in metric_logger.log_every(data_loader, 100, header):
            ...
            for i in range(images.shape[0]):
                ...
                #cal kl 
                kl = kldiv(gaze_average, output).item()
                kl_list.append(kl)
            ...
            for idx in top_kl_idx:
                for j in range(batch_size - topK):
                    mix_imgs, mix_ps =  mixup_data(images, p, attention_list, idx, j+topK)
...
def main(args):
    ...
    # val_noise_dataset_gauss = SceneDataset(args.data_path, mode='val', noise_type='gaussian_noise')
    # val_noise_dataset_motion = SceneDataset(args.data_path, mode='val', noise_type='motion_blur')
    ...
    for epoch in tqdm(range(args.start_epoch, args.epochs)):
        ...
        if epoch == 0:
            init_infer_dataset = SceneDataset(args.data_path, mode='infer', p_dic = args.p_dic,  alpha=args.alpha)
       
            init_infer_dataloader = data.DataLoader(init_infer_dataset,
                                        batch_size=args.batch_size,  
                                        num_workers=num_workers,
                                        pin_memory=True)
            run_infer(args, model, init_infer_dataloader, device='cuda')
        ...
        # infer_dataset = ConcatDataset([init_infer_dataset, mix_infer_dataset])
        ...
        run_infer_mixup(args, model, init_infer_dataloader, gaze_average, device='cuda') #mixup
        kl_db, gaze_average = init_train_dataset.cal_data_kl(name=args.name)
        csv_file_path = './' + args.name  + '_{}'.format(epoch) + '.csv'
