...
class SceneDataset(Dataset):
    ...
    def calculate_cc(self, tensor_i, tensor_j):
        """
        计算两个tensor之间的相关系数（cc）。
        """
        tensor_i_flat = tensor_i.flatten()
        tensor_j_flat = tensor_j.flatten()
    ...
    def __getitem__(self, i):
        ...
        cc = self.calculate_cc(p_list[0], gaze)
...
if __name__ == '__main__':
    ...
    sample_num = 1000
    cc_threshold = 0.8
    p_dics = ['ml_p', 'unisal_p', 'sam_p',  'tased_p']
    p_samples = [ [] for i in range(4) ]
    
    
    def calculate_difference(a, b):
        """
        计算两个列表之间的差异，具体为 |a| - |a与b的交集|
        ...
        difference = len(set_a) - len(intersection)
        return difference
    # import pdb; pdb.set_trace()
    for index, p_dic in enumerate(p_dics):
        dataset = SceneDataset("../../atten_data/bd", mode='train', p_dic = [p_dic], sample_num=sample_num, cc_threshold=cc_threshold)
        for i in range(len(dataset)):
            p_samples[index].append(dataset[i])
        p_samples[index] = [x for x in p_samples[index] if x != -1]
        print("p: {}, len: {}".format(p_dic, len(p_samples[index])))
    matrix_res = torch.zeros(4, 4)
    for i in range(4):
        for j in range(4):
            if i!=j:
                matrix_res[i, j] = calculate_difference(p_samples[i], p_samples[j])
    # matrix_res = torch.zeros(4, 4)
    # from tqdm import tqdm
    ...
    torch.save(matrix_res, 'differ_matrix_res.pt')
    print('matrix_res: {}'.format(matrix_res))
    ...
    plt.savefig('differ_matrix_res_heatmap.png')
    plt.close()
    # dataloader = DataLoader(dataset,
    #                         batch_size=32,
    #                         num_workers=8,
